...
function Initialize(){
    ...
    inputs.innerHTML +=`<input type="text" id="inputTEXT"/>`;
    inputs.innerHTML +=`<button id="button" onclick="Calculate()" >Run</button>`;
}



function Calculate(){//This function outputs the answer based on input
    ...
    outputString= Calculations(fromFrom,fromAmount,fromPeriods,fromRate);
    //outputs the answer
...
function Calculations(toFrom, toAmount, toPeriods, toRate){
   var type = toFrom.toUpperCase();
   var amount = toAmount;
   var n = toPeriods;
   var i = toRate;
   var ansr = 0;
   tense = "";
   //System.out.println(\"Type= \" + type + \" , Amount= \" + amount + \", Periods= \" + n + \", Interest= \" + i);
    ...
    else {ansr="Please reread the directions."; }
    return ansr
}
